...
LLAMACPP_DRI_RENDER="${LLAMACPP_DRI_RENDER}" docker compose --env-file .env.advanced \
  build llamacpp
...
docker exec ws-llamacpp sh -lc 'uname -m; ls -l /usr/share/vulkan/icd.d || true; cat /usr/share/vulkan/icd.d/radeon_icd.json 2>/dev/null || true; ls -l /usr/lib/*/libvulkan_radeon.so 2>/dev/null || true; vulkaninfo --summary' | head -n 120 || true
